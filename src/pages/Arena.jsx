...
const analysisProfileMap = {
  ...
  memory_chain: {
    title: "Memory Chain Analysis Matrix",
    analysisLines: [
      "Ordered recall stable",
      "Chain integrity retained",
      "Missing-link recovery consistent",
    ],
    tone: "sequenced",
    accentColor: "#22d3ee",
...
const buildMemoryChainResultsCopy = ({
  accuracy = 0,
  correctAnswers = 0,
} = {}) => {
  if (accuracy >= 90) {
    return {
      ...
      title: "Memory Chain Results",
      summary:
        "Excellent ordered recall. The chain stayed intact and the missing link was recovered with confidence.",
    };
  }
  ...
  if (accuracy >= 70) {
    return {
      ...
      title: "Memory Chain Results",
      summary:
        "Strong chain tracking. The ordered sequence remained readable and the hidden value was close at hand.",
    };
  }
  ...
  if (correctAnswers > 0) {
    return {
      ...
      title: "Memory Chain Results",
      summary:
        "Partial progress. The sequence held shape, but the missing link needs tighter recall.",
    };
  }
  ...
  return {
    ...
    title: "Memory Chain Results",
    summary:
      "Chain timed out. Rebuild the order from the start and focus on the hidden position.",
  };
};
...
const MEMORY_CHAIN_PUZZLE_META = {
  label: "Memory Chain",
  shortLabel: "Chain",
...
function Arena({ theme }) {
  ...
  const memoryChainPuzzleMetrics = useMemo(() => {
    if (!isMemoryChainPuzzle) {
      return {};
    }

    return {
      difficulty: currentPuzzle?.difficulty || currentPuzzle?.difficultyBucket || "medium",
      chainLength: Array.isArray(currentPuzzle?.sequence) ? currentPuzzle.sequence.length : 0,
      hiddenIndex: Number.isInteger(currentPuzzle?.hiddenIndex) ? currentPuzzle.hiddenIndex : null,
                    ...
                    <div className="relative z-10">
                      <div className="mb-6 flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
                        <div>
                          ...
                          <h3 className={`text-2xl font-bold ${isCyber ? "text-white text-glow-blue" : "text-white"}`}>
                            {currentPuzzle?.prompt || "Identify the hidden value."}
                          </h3>
                      ...
                      <div className="mt-6 grid w-full gap-6 lg:grid-cols-[minmax(0,1fr)_360px] lg:items-start">
                        ...
                        <div className="xl:sticky xl:top-6">
                          <div className="rounded-2xl border border-fuchsia-500/10 bg-slate-950/60 p-5 shadow-[inset_0_0_35px_rgba(2,6,23,0.6),0_0_30px_rgba(217,70,239,0.12)]">
                            ...
                            <div className="mt-4 flex flex-col items-center gap-3">
                              ...
                              <div className="flex w-full flex-wrap justify-center gap-3 md:gap-4">
                                {(Array.isArray(currentPuzzle?.options) ? currentPuzzle.options : [currentPuzzle?.answer])
                          ...
                          {SHOW_PUZZLE_DEBUG_META && (
                            <div className="mt-4">
                              <DevDebugPanel title="Memory Chain Dev">
                                {SHOW_ANSWERS && (
                                  <div>
                                    <span className="font-bold text-white">Answer:</span>{" "}
                                ...
                                <div>
                                  ...
                                  <span className="text-amber-100">
                                    {Number.isInteger(currentPuzzle?.hiddenIndex) ? currentPuzzle.hiddenIndex : "—"}
                                  </span>
                                </div>
                                <div>
